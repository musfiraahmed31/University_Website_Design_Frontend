...
// =============================================
// 8. EMAILJS CONFIGURATION (SIMPLIFIED)
// =============================================
(function() {
    // Initialized with your specific Public Key
...
const timeInput = document.getElementById('timeValue'); // To capture the {{time}} variable

if (mainContactForm) {
    mainContactForm.addEventListener('submit', function(e) {
        e.preventDefault();
        
        // Grab the current date/time to send to the {{time}} variable in your template
        if(timeInput) {
            const now = new Date();
            timeInput.value = now.toLocaleString();
        }

        // Get text values for basic validation
        ...
        // Update button state to show it is processing
        formSubmitBtn.innerHTML = '<span>Sending...</span> <i class="fas fa-spinner fa-spin"></i>';
        formSubmitBtn.disabled = true;
        formStatusMessage.textContent = '';
        
        // Send using sendForm
        // Parameters: Service ID, Template ID, HTML Form Element
        emailjs.sendForm('service_fq9r83n', 'template_pd9ltcp', this)
            .then(function(response) {
                console.log('Email sent successfully!', response.status, response.text);
                
                displayFormStatus('Thank you! Your inquiry has been submitted.', 'success');
                ...
                displayFormStatus('Failed to send message. Please check your connection and try again.', 'error');
                resetFormButton();
            });
    });
}
...
function resetFormButton() {
    formSubmitBtn.innerHTML = 'Submit <i class="fas fa-paper-plane"></i>';
